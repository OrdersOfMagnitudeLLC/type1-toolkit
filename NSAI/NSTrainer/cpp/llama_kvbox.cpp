...
static int llama_decode_internal(
    ...
    for (uint32_t cur_token = 0; cur_token < n_tokens_all; ) {
        ...
        // KVBox retrieval: inject historical context into working cache before
        // graph compute.
        // Prefill (n_tokens > 1): inject ALL KVBox positions into free slots.
        // Decode (n_tokens == 1): inject up to 16 positions, evict oldest if needed.
        struct KVInjectState {
            uint32_t cell_idx;
            llama_pos orig_pos;
            std::vector<std::vector<uint8_t>> k_saved;  // [layer] -> K bytes
            std::vector<std::vector<uint8_t>> v_saved;  // [layer] -> V bytes
        };
        std::vector<KVInjectState> kv_injects;

        if (cparams.kv_box && lctx.kv_box.initialized() && !kv_self.recurrent) {
            const bool is_prefill = (n_tokens > 1);

            // Count free slots in working cache
            ...
            // Determine current working window
            llama_pos cur_pos = 0;
            for (int i = 0; i < n_tokens; ++i) {
                if (u_batch.pos[i] > cur_pos) cur_pos = u_batch.pos[i];
            }
            const llama_pos window_start = (cur_pos >= (llama_pos)kv_self.size)
                                         ? cur_pos - (llama_pos)kv_self.size + 1
                                         : 0;

            // Get all KVBox positions outside the working window
            ...
            for (int64_t p : all_positions) {
                if (p < (int64_t)window_start) {
                    candidates.push_back((llama_pos)p);
                }
            }
            ...
            if (candidates.empty()) {
                ...
                // PREFILL: evict 16 oldest cells (not in current ubatch),
                // save to KVBox, inject 16 stride-sampled historical positions
                const uint32_t max_inject = 128; // prefill: 128 injection slots

                // Build set of current ubatch positions (don't evict these)
                std::unordered_set<llama_pos> ubatch_pos;
                for (int i = 0; i < n_tokens; ++i) {
                    ubatch_pos.insert(u_batch.pos[i]);
                }

                // K-vector rolling-mean novelty scoring: for each candidate cell
 ...
 // their local context and are PROTECTED (never evicted - they stay
                ...
                std::vector<CellPos> all_candidates;
                for (uint32_t j = 0; j < kv_self.n; ++j) {
                    llama_pos cp = kv_self.cells[j].pos;
                    if (cp >= 0 && ubatch_pos.find(cp) == ubatch_pos.end()) {
                        all_candidates.push_back({j, cp, 0.0f});
                    }
                }
                std::sort(all_candidates.begin(), all_candidates.end(),
                    [](const CellPos & a, const CellPos & b) { return a.pos < b.pos; });

                std::vector<CellPos> evict_candidates; // below-threshold: KVBox eviction candidates
                const int il_s = 35;
                const uint32_t rolling_window = 64;
                const float protect_sigma = 1.4f;
                if (!all_candidates.empty() && (int)kv_self.k_l.size() > il_s && kv_self.k_l[il_s]
                    && kv_self.k_l[il_s]->type == GGML_TYPE_F16
                    && lctx.cache_copies[2*il_s+0].step > 0) {
                    const size_t k_step_s = lctx.cache_copies[2*il_s+0].step;
                    const uint32_t n_heads_s = hparams.n_head_kv(il_s);
                    const uint32_t hdim_s    = hparams.n_embd_head_k(il_s);
                    const size_t k_elem = (size_t)n_heads_s * hdim_s;

                    std::vector<std::vector<float>> k_vecs(all_candidates.size());
                    for (size_t i = 0; i < all_candidates.size(); ++i) {
                        const ggml_fp16_t* k = (const ggml_fp16_t*)
                            ((char*)kv_self.k_l[il_s]->data + (size_t)all_candidates[i].idx * k_step_s);
                        k_vecs[i].resize(k_elem);
                        for (size_t d = 0; d < k_elem; ++d) k_vecs[i][d] = ggml_fp16_to_fp32(k[d]);
                    }

                    // Rolling-mean cosine distance from the preceding `rolling_window` tokens
                    std::vector<float> dists(all_candidates.size(), 0.0f);
                    for (size_t i = 0; i < all_candidates.size(); ++i) {
                        size_t start = (i >= rolling_window) ? (i - rolling_window) : 0;
                        if (start == i) { dists[i] = 0.0f; continue; } // no history yet
                        std::vector<double> mean_k(k_elem, 0.0);
                        uint32_t cnt = 0;
                        for (size_t j = start; j < i; ++j) {
                            for (size_t d = 0; d < k_elem; ++d) mean_k[d] += k_vecs[j][d];
                            cnt++;
                        }
                        for (auto & v : mean_k) v /= cnt;

                        double dot = 0.0, norm_k = 0.0, norm_m = 0.0;
                        for (size_t d = 0; d < k_elem; ++d) {
                            float kv = k_vecs[i][d];
                            float mv = (float)mean_k[d];
                            dot += kv * mv;
                            norm_k += kv * kv;
                            norm_m += mv * mv;
                        }
                        float cos_sim = (norm_k > 0 && norm_m > 0)
                            ? (float)(dot / (sqrt(norm_k) * sqrt(norm_m)))
                            : 0.0f;
                        dists[i] = 1.0f - cos_sim;
                    }

                    // Mean / stddev over cells that had a full rolling window
                    double sum_d = 0.0, sum_d2 = 0.0;
                    uint32_t n_d = 0;
                    for (size_t i = rolling_window; i < dists.size(); ++i) {
                        sum_d += dists[i];
                        sum_d2 += (double)dists[i] * dists[i];
                        n_d++;
                    }
                    float mean_d = n_d > 0 ? (float)(sum_d / n_d) : 0.0f;
                    float var_d  = n_d > 0 ? (float)(sum_d2 / n_d - (double)mean_d * mean_d) : 0.0f;
                    float std_d  = var_d > 0 ? sqrtf(var_d) : 0.0f;
                    float threshold = mean_d + protect_sigma * std_d;

                    printf("[KVBox prefill] K-novelty: mean=%.4f std=%.4f threshold(%.1fsigma)=%.4f (n=%u)\n",
                           mean_d, std_d, protect_sigma, threshold, n_d);

                    uint32_t n_protected = 0;
                    for (size_t i = 0; i < all_candidates.size(); ++i) {
                        all_candidates[i].entropy = dists[i];
                        bool is_protected = (i >= rolling_window) && (dists[i] > threshold);
                        if (all_candidates[i].pos >= 1200 && all_candidates[i].pos <= 1220) {
                            printf("[KVBox prefill] MANGO area pos=%d k_dist=%.4f %s\n",
                                   all_candidates[i].pos, dists[i], is_protected ? "PROTECTED" : "evict-candidate");
                        }
                        if (is_protected) {
                            n_protected++;
                        } else {
                            evict_candidates.push_back(all_candidates[i]);
                        }
                    }
                    printf("[KVBox prefill] K-novelty protection: %u protected, %zu evict-candidates (of %zu total)\n",
                           n_protected, evict_candidates.size(), all_candidates.size());
                } else {
                    evict_candidates = all_candidates;
                }

                // Sort by entropy DESCENDING (most unique first = save to KVBox)
                std::sort(evict_candidates.begin(), evict_candidates.end(),
                    [](const CellPos & a, const CellPos & b) { return a.entropy > b.entropy; });

                // Save ALL candidates to KVBox (not just the ones we'll evict for injection).
                // Un-rotate K to pre-RoPE before storing so position isn't baked in.
                {
                    const auto rope_freqs_w = kvbox_rope_freqs(hparams, cparams);
                    const uint32_t n_rot_w = hparams.n_rot;
                    const int n_layer_kv = model.mtp ? (int)hparams.n_layer
                                                     : (int)hparams.n_layer - (int)hparams.nextn_predict_layers;
                    std::vector<ggml_fp16_t> k_pre_w;
                    for (const auto & c : evict_candidates) {
                        for (int il = 0; il < n_layer_kv; ++il) {
                            if (hparams.is_recurrent(il)) continue;
                            ...
                            const ggml_fp16_t* ek = (const ggml_fp16_t*)
                                ((char*)kv_self.k_l[il]->data + (size_t)c.idx * k_step);
                            const ggml_fp16_t* ev = has_v ? (const ggml_fp16_t*)
                                ((char*)kv_self.v_l[il]->data + (size_t)c.idx * v_step) : nullptr;
                            k_pre_w.resize(hdim_il);
                            for (uint32_t h = 0; h < n_heads_il; ++h) {
                                memcpy(k_pre_w.data(), ek + h * hdim_il, hdim_il * sizeof(ggml_fp16_t));
                                kvbox_rope_k(k_pre_w.data(), hdim_il, -(int64_t)c.pos, rope_freqs_w, n_rot_w);
                                lctx.kv_box.write_slot((uint64_t)c.pos, il, h,
                                    k_pre_w.data(), ev ? ev + h * hdim_il : k_pre_w.data());
                            }
                        }
                    }
                    printf("[KVBox prefill] saved ALL %u candidates to KVBox (pre-RoPE K)\n", (uint32_t)evict_candidates.size());
                }

                // Debug: check MANGO status
                for (const auto & c : evict_candidates) {
                    if (c.pos >= 1200 && c.pos <= 1220) {
                        printf("[KVBox prefill] MANGO pos=%d saved to KVBox (entropy=%.4f)\n", c.pos, c.entropy);
                    }
                }

                // Only evict max_inject cells for injection (top entropy = most unique)
                uint32_t n_evict = std::min((uint32_t)evict_candidates.size(), max_inject);
                evict_candidates.resize(n_evict);

                // Also use any free cells
                uint32_t n_total_slots = n_free + n_evict;
                uint32_t n_inject = std::min((uint32_t)candidates.size(), n_total_slots);
                if (n_inject > 0) {
                    // Dense local windows: 8 blocks of 16 contiguous positions
                    const uint32_t block_size = 16;
                    const uint32_t n_blocks = n_inject / block_size;
                    uint32_t block_stride = n_blocks > 0
                        ? (uint32_t)candidates.size() / n_blocks
                        : (uint32_t)candidates.size();
                    if (block_stride < block_size) block_stride = block_size;
                    ...
                    const auto rope_freqs_inj = kvbox_rope_freqs(hparams, cparams);
                    const uint32_t n_rot_inj = hparams.n_rot;

                    // Build target list: free cells first, then evicted cells
                    struct InjectTarget {
                        uint32_t cell_idx;
                        llama_pos orig_pos;  // -1 for free slot
                    };
                    std::vector<InjectTarget> targets;
                    for (uint32_t i = 0; i < n_free && targets.size() < n_inject; ++i) {
                        targets.push_back({free_cells[i], -1});
                    }
                    for (uint32_t i = 0; i < n_evict && targets.size() < n_inject; ++i) {
                        targets.push_back({evict_candidates[i].idx, evict_candidates[i].pos});
                    }

                    for (uint32_t i = 0; i < n_inject; ++i) {
                        uint32_t cell_idx = targets[i].cell_idx;
                        llama_pos orig_pos = targets[i].orig_pos;
                        // Dense block: block_idx = i / block_size, offset = i % block_size
                        uint32_t block_idx = i / block_size;
                        uint32_t offset    = i % block_size;
                        uint32_t base_idx  = block_idx * block_stride;
                        uint32_t cand_idx  = base_idx + offset;
                        if (cand_idx >= candidates.size()) cand_idx = (uint32_t)candidates.size() - 1;
                        llama_pos abs_pos = candidates[cand_idx];

                        KVInjectState inj;
                        inj.cell_idx = cell_idx;
                        inj.orig_pos = orig_pos;
                        inj.k_saved.resize(n_layer_kv);
                        inj.v_saved.resize(n_layer_kv);
                        ...
                        for (int il = 0; il < n_layer_kv; ++il) {
                            ...
                            const uint32_t n_heads_il = hparams.n_head_kv(il);
                            const uint32_t hdim_il    = hparams.n_embd_head_k(il);
                            const size_t kv_bytes = (size_t)n_heads_il * hdim_il * sizeof(ggml_fp16_t);

                            bool has_v = !kv_self.v_l.empty() && il < (int)kv_self.v_l.size() && kv_self.v_l[il];
                            size_t v_step = 0;
                            if (has_v) {
                                v_step = lctx.cache_copies[2*il+1].step;
                                if (v_step < kv_bytes) has_v = false;
                            }

                            // For evicted cells: save K/V to KVBox and to inj for restoration
                            if (orig_pos >= 0) {
                                inj.k_saved[il].resize(kv_bytes);
                                memcpy(inj.k_saved[il].data(),
                                    (char*)kv_self.k_l[il]->data + (size_t)cell_idx * k_step, kv_bytes);
                                if (has_v) {
                                    inj.v_saved[il].resize(kv_bytes);
                                    memcpy(inj.v_saved[il].data(),
                                        (char*)kv_self.v_l[il]->data + (size_t)cell_idx * v_step, kv_bytes);
                                }
                                // Save evicted cell to KVBox (un-rotate K to pre-RoPE)
                                const ggml_fp16_t* ek = (const ggml_fp16_t*)
                                ...
                                for (uint32_t h = 0; h < n_heads_il; ++h) {
                                    ...
                                    kvbox_rope_k(k_pre_evict.data(), hdim_il, -(int64_t)orig_pos, rope_freqs_inj, n_rot_inj);
                                    lctx.kv_box.write_slot((uint64_t)orig_pos, il, h,
                                        k_pre_evict.data(), ev ? ev + h * hdim_il : k_pre_evict.data());
                                }
                            }

                            // Inject historical position from KVBox.
                            // Apply RoPE at abs_pos to the stored pre-RoPE K.
                            for (uint32_t h = 0; h < n_heads_il; ++h) {
                                if (!lctx.kv_box.read_slot((uint64_t)abs_pos, il, h, k_buf.data(), v_buf.data())) continue;
                                any_hit = true;
                                kvbox_rope_k(k_buf.data(), hdim_il, (int64_t)abs_pos, rope_freqs_inj, n_rot_inj);
                        ...
                        if (any_hit) {
                            ...
                            lctx.kv_box.prefill_retrievals++;
                            kv_injects.push_back(std::move(inj));
                        }
                    }
                }
            }
            // Decode-step injection removed: now handled post-prefill above.

            // Update kv_self.n to cover injected positions so the graph
            // includes them in the attention mask
            if (!kv_injects.empty()) {
                const uint32_t pad = llama_kv_cache::get_padding(cparams.flash_attn);
                uint32_t max_cell = 0;
                for (uint32_t j = 0; j < kv_self.n; ++j) {
                    if (kv_self.cells[j].pos >= 0 && j > max_cell) {
                        max_cell = j;
                    }
                }
                for (const auto & inj : kv_injects) {
                    if (inj.cell_idx > max_cell) {
                        max_cell = inj.cell_idx;
                    }
                }
        ...
        // KVBox: mirror KV writes to compressed cache (store pre-RoPE K)
        ...
        // Keep prefill Q (from question) for all decode scoring steps
        if (cparams.kv_box && lctx.kv_box.initialized() && n_tokens > 1) {
            ...
            // Find the post-RoPE Q tensor from a deeper layer (KVBOX_RETRIEVAL_LAYER)
            // instead of layer 0. Layer-0 Q is too shallow/surface-level to
            // discriminate semantically relevant content from noise; a
            // mid-to-late layer's Q carries much richer contextual signal.
            ...
            if (q_roped) {
                // Qcur shape: [head_dim, n_heads, n_tokens]
        ...
        // KVBox: restore injected cells back to original state after graph compute
        for (const auto & inj : kv_injects) {
            if (inj.orig_pos >= 0) {
                // Evicted cell: restore full-precision K/V
                const int n_layer_kv = model.mtp ? (int)hparams.n_layer
                                                 : (int)hparams.n_layer - (int)hparams.nextn_predict_layers;
                for (int il = 0; il < n_layer_kv; ++il) {
                    if (inj.k_saved[il].empty()) continue;
                    const size_t k_step = lctx.cache_copies[2*il+0].step;
                    if (k_step == 0) continue;

                    memcpy((char*)kv_self.k_l[il]->data + (size_t)inj.cell_idx * k_step,
                           inj.k_saved[il].data(), inj.k_saved[il].size());

                    if (!inj.v_saved[il].empty()) {
                        size_t v_step = lctx.cache_copies[2*il+1].step;
                        if (v_step > 0) {
                            memcpy((char*)kv_self.v_l[il]->data + (size_t)inj.cell_idx * v_step,
                                   inj.v_saved[il].data(), inj.v_saved[il].size());
                        }
                    }
                }
                kv_self.cells[inj.cell_idx].pos = inj.orig_pos;
                kv_self.cells[inj.cell_idx].seq_id.clear();
                kv_self.cells[inj.cell_idx].seq_id.insert(0);
            } else {
                // Free slot: just clear the cell
                kv_self.cells[inj.cell_idx].pos = -1;
                kv_self.cells[inj.cell_idx].seq_id.clear();
            }
        }

        // Post-prefill KVBox injection: score KVBox positions using the captured
        // retrieval_q and inject top-384 into the working cache BEFORE any decode
        // token is processed. This runs once after prefill completes; subsequent
        // decode steps skip injection entirely (decode_injected_once = true).
        if (cparams.kv_box && lctx.kv_box.initialized() && n_tokens > 1
                && !lctx.kv_box.retrieval_q.empty()
                && n_tokens < (int)cparams.n_batch) {
            ...
            // Recompute free cells and cache positions after kv_injects restore
            std::vector<uint32_t> free_cells;
            ...
            // Build ubatch position set
            std::unordered_set<llama_pos> ubatch_pos_set;
            llama_pos cur_pos = 0;
            for (int i = 0; i < n_tokens; ++i) {
                ubatch_pos_set.insert(u_batch.pos[i]);
                if (u_batch.pos[i] > cur_pos) cur_pos = u_batch.pos[i];
            }

            // Find eviction candidates (oldest cells, keep only last token)
            struct CellPos { uint32_t idx; llama_pos pos; };
            std::vector<CellPos> evict_cells;
            if (n_free < max_inject) {
                for (uint32_t j = 0; j < kv_self.n; ++j) {
                    llama_pos cp = kv_self.cells[j].pos;
                    if (cp >= 0 && ubatch_pos_set.find(cp) == ubatch_pos_set.end()) {
            ...
            // Remove evicted positions from cache_pos_set so they can be re-injected
            ...
            for (int64_t p : all_positions) {
                ...
                if (cache_pos_set.find(lp) == cache_pos_set.end()
                    && ubatch_pos_set.find(lp) == ubatch_pos_set.end()) {
            ...
            std::vector<ScoredPos> scored;
            {
                ...
                for (llama_pos lp : candidates) {
                    ...
                    for (uint32_t h_kv = 0; h_kv < n_heads_kv; h_kv++) {
                        if (!lctx.kv_box.read_score_k((uint64_t)lp, h_kv, kbox_k.data())) continue;
                        any_hit = true;
                        kvbox_rope_k(kbox_k.data(), head_dim,
                                     lctx.kv_box.retrieval_q_pos,
            ...
            // Debug output
            if (!scored.empty()) {
                printf("[KVBox post-prefill] top-5: ");
                for (uint32_t i = 0; i < std::min((uint32_t)scored.size(), 5u); i++) {
                    printf("%d(%.1f) ", scored[i].pos, scored[i].score);
                }
                printf("| total scored=%zu\n", scored.size());
                for (const auto & s : scored) {
                    if (s.pos >= 1205 && s.pos <= 1215) {
                        printf("[KVBox post-prefill] MANGO7734 area: pos=%d score=%.1f\n", s.pos, s.score);
                    }
                }
            }
            ...
            if (!scored.empty()) {
                ...
                for (uint32_t i = 0; i < scored.size(); i++) {
                    if (scored[i].pos >= 1200 && scored[i].pos <= 1220) {
                        printf("[KVBox post-prefill] POST-SMOOTH pos=%d rank=%u score=%.1f\n",
                               scored[i].pos, i, scored[i].score);
                    }
                }
            }
