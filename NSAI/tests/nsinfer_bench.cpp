...
SparseResult sparse_swiglu_blas(
    ...
    // Step 2: Compute blended energy signals (cache for reuse)
    float* contributions = arena.alloc(d_ff);
    float total_energy = 0.0f;
    float max_energy = 0.0f;
    for (int j = 0; j < d_ff; ++j) {
        float norm_factor = down_row_norms[j] / mean_down_norm;
        contributions[j] = gate_out[j] * gate_out[j] * (1.0f + 0.2f * norm_factor);
    ...
    // Pack active columns of up_proj [d_model x selected_k]
    float* active_up = arena.alloc(d_model * selected_k);
    for (int i = 0; i < d_model; ++i) {
        for (int k = 0; k < selected_k; ++k) {
            int j = active_indices[k];
            active_up[i * selected_k + k] = up_proj[i * d_ff + j];
        }
    }
    
    // Extract active gate values (already computed)
    float* active_gate_vals = arena.alloc(selected_k);
    for (int k = 0; k < selected_k; ++k) {
        active_gate_vals[k] = gate_out[active_indices[k]];
    }
    
    // Pack active columns of down_proj [selected_k x d_model]
    ...
    // up_out = x @ active_up
    naive_sgemm_rowmajor(1, selected_k, d_model, x, active_up, up_out);
    
    // hidden = gate_out * up_out (elementwise)
    for (int k = 0; k < selected_k; ++k) {
        hidden[k] = active_gate_vals[k] * up_out[k];
    }
...
int main() {
    ...
    load_weights((base + "/NS/NSInfer/weights/gate_proj.bin").c_str(), gate_proj, d_model * d_ff);
    load_weights((base + "/NS/NSInfer/weights/up_proj.bin").c_str(), up_proj, d_model * d_ff);
    load_weights((base + "/NS/NSInfer/weights/down_proj.bin").c_str(), down_proj, d_ff * d_model);
