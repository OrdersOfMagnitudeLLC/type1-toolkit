...
#ifndef NS_IQ1_S_QUANT_H
#define NS_IQ1_S_QUANT_H
...
#define NS_IQ1S_BLOCK_SIZE 32
#define NS_IQ1S_DELTA     0.125f
#define NS_NGRID_IQ1S     2048
#define NS_GROUP_MAX_EPS_IQ1_S 1e-12f

// IQ1_S block: 256 values, fp16 super-scale + 32B 1-bit quants + 16B high bits
typedef struct {
    ggml_half d;
    uint8_t  qs[QK_K/8];
    uint16_t qh[QK_K/32];
...
static const uint16_t kgrid_1bit_2048[NS_NGRID_IQ1S] = {
...
// iq2_data for the IQ1_S grid only (gindex 2 in the original 4-entry table).
struct Iq1sGrid {
    uint64_t * grid       = nullptr;
    int      * map        = nullptr;
    uint16_t * neighbours = nullptr;
};
static Iq1sGrid g_iq1s;
static std::once_flag g_iq1s_once;
...
// Port of iq2xs_init_impl(GGML_TYPE_IQ1_S): builds grid + kmap + neighbours.
static void init_impl() {
    const int grid_size = NS_NGRID_IQ1S;
    const int kmap_size = 43692;
    const int nwant = 3;
    const uint16_t * kgrid = kgrid_1bit_2048;
    ...
    g_iq1s.neighbours = kneighbors_q2xs;
    int counter = 0;
    for (int i = 0; i < kmap_size; ++i) {
        if (kmap_q2xs[i] >= 0) continue;
        for (int k = 0; k < 8; ++k) {
...
// Thread-safe one-time init. Called from quantize_row_iq1_s.
static inline void ensure_init() {
    std::call_once(g_iq1s_once, init_impl);
}
...
static void iq1s_process_1block(int block_size, const float * xb, const float * weight, int8_t * L, float * the_scale, uint16_t * the_index, int * the_shift,
        float * pairs, float * sumx, float * sumw) {
    float max = fabsf(xb[0]);
    for (int i = 1; i < block_size; ++i) max = MAX(max, fabsf(xb[i]));
    if (max < NS_GROUP_MAX_EPS_IQ1_S) {
        *the_scale = 0;
        *the_shift = 1;
        for (int k = 0; k < block_size/8; ++k) the_index[k] = 0;
        return;
    }
    const uint64_t * kgrid_q2xs      = g_iq1s.grid;
    const int      * kmap_q2xs       = g_iq1s.map;
    const uint16_t * kneighbors_q2xs = g_iq1s.neighbours;

    const float x_p[3] = {-1 + NS_IQ1S_DELTA,  NS_IQ1S_DELTA, 1 + NS_IQ1S_DELTA};
    const float x_m[3] = {-1 - NS_IQ1S_DELTA, -NS_IQ1S_DELTA, 1 - NS_IQ1S_DELTA};
    ...
    for (int k = 0; k < block_size/8; ++k) {
        ...
        if (grid_index < 0) {
            ...
            grid_index = iq1_find_best_neighbour2(neighbours, kgrid_q2xs, xb + 8*k, weight + 8*k, scale, xx, L + 8*k, NS_NGRID_IQ1S);
            assert(grid_index >= 0);
        }
...
static void quantize_row_iq1_s_impl(const float * GGML_RESTRICT x, void * GGML_RESTRICT vy, int64_t n, const float * GGML_RESTRICT quant_weights,
    ...
    const int      * kmap_q2xs       = g_iq1s.map;
    const uint16_t * kneighbors_q2xs = g_iq1s.neighbours;
    (void)kgrid_q2xs; (void)kmap_q2xs; (void)kneighbors_q2xs;
    ...
    const int block_size = NS_IQ1S_BLOCK_SIZE;
    ...
    for (int ibl = 0; ibl < nbl; ++ibl) {
        ...
        y[ibl].d = GGML_FP32_TO_FP16(d*1.125f); // 1.125f fudge factor, matches ik_llama
        float id = 1/d;
        for (int ib = 0; ib < QK_K/block_size; ++ib) {
...
// Public entry: quantize k floats (k % 256 == 0) into block_iq1_s blocks.
// Uses unit quant weights (same as quantize_row_iq1_s_ref in ik_llama).
inline void quantize_row_iq1_s(const float * GGML_RESTRICT x, void * GGML_RESTRICT vy, int64_t k) {
    ns_iq1s::ensure_init();
    assert(k % QK_K == 0);
    const int64_t nbl = k / QK_K;
    block_iq1_s * y = (block_iq1_s *)vy;

    float  scales[QK_K/NS_IQ1S_BLOCK_SIZE];
    float  weight[NS_IQ1S_BLOCK_SIZE];
    int8_t L[NS_IQ1S_BLOCK_SIZE];
    float  sumx[NS_IQ1S_BLOCK_SIZE+1];
    float  sumw[NS_IQ1S_BLOCK_SIZE+1];
    float  pairs[2*NS_IQ1S_BLOCK_SIZE];
    uint16_t index[NS_IQ1S_BLOCK_SIZE/8];
    int8_t shifts[QK_K/NS_IQ1S_BLOCK_SIZE];
    float  qw[QK_K];
    for (int j = 0; j < QK_K; ++j) qw[j] = 1;

    for (int64_t ibl = 0; ibl < nbl; ++ibl) {
        ns_iq1s::quantize_row_iq1_s_impl(x + ibl*QK_K, &y[ibl], QK_K, qw, scales, weight, sumx, sumw, pairs, L, index, shifts);
    }
}

#endif // NS_IQ1_S_QUANT_H
