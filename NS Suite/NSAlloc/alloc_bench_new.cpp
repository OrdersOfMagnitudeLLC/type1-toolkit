...
void pattern_a_sequential() {
    std::cout << "Pattern A - Sequential:" << std::endl;
    ...
    // NSMultiPool
    {
        ...
        for (size_t i = 0; i < ptrs.size(); ++i) {
            alloc.free(ptrs[i], alloc_sizes[i]);
        }
        auto end = std::chrono::high_resolution_clock::now();
        
        double ns_per_op = std::chrono::duration<double, std::nano>(end - start).count() / (ptrs.size() * 2);
...
void pattern_b_clustered_free() {
    std::cout << "Pattern B - Clustered free:" << std::endl;
    ...
    // NSMultiPool
    {
        ...
        // Free in clusters: every other 100-block cluster
        for (size_t cluster = 0; cluster < NUM_OPS / 100; cluster += 2) {
            for (size_t i = cluster * 100; i < (cluster + 1) * 100; ++i) {
                alloc.free(ptrs[i], alloc_sizes[i]);
            }
        }
        
        // Reallocate 50K blocks
        auto start = std::chrono::high_resolution_clock::now();
        ...
        double ns_per_op = std::chrono::duration<double, std::nano>(end - start).count() / 50000;
        std::cout << "  NSMultiPool: " << ns_per_op << " ns/op" << std::endl;
    }
...
void pattern_c_random() {
    std::cout << "Pattern C - Random:" << std::endl;
    
    std::mt19937 rng_free(43);
    std::uniform_int_distribution<size_t> free_dist(0, NUM_OPS - 1);
    ...
    // NSMultiPool
    {
        ...
        // Free 50K random blocks
        std::vector<bool> freed(NUM_OPS, false);
        size_t freed_count = 0;
        while (freed_count < 50000) {
            size_t idx = free_dist(rng_free);
            if (!freed[idx] && ptrs[idx]) {
                alloc.free(ptrs[idx], alloc_sizes[idx]);
                ptrs[idx] = nullptr;
                freed[idx] = true;
                freed_count++;
            }
        ...
        for (size_t i = 0; i < NUM_OPS && reallocated < 50000; ++i) {
            if (!ptrs[i]) {
                size_t size = sizes[i % 5];
                void* p = alloc.alloc(size);
                if (!p) {
...
void pattern_e_fixed_size_hot_path() {
    ...
    // NSPool<64, 4096>
    {
        ...
        double ns_per_op = std::chrono::duration<double, std::nano>(end - start).count() / (100000 * 4096 * 2);
        std::cout << "  NSPool<64, 4096>: " << ns_per_op << " ns/op" << std::endl;
    }
...
int main() {
    pattern_a_sequential();
    pattern_b_clustered_free();
    pattern_c_random();
