...
private:
    NSPool<8,   20480> p8_;
    NSPool<16,  20480> p16_;
    NSPool<32,  20480> p32_;
    NSPool<64,  20480> p64_;
    NSPool<128, 20480> p128_;

public:
    void* alloc(size_t size) {
        if (size <= 8)   return p8_.alloc();
        if (size <= 16)  return p16_.alloc();
        if (size <= 32)  return p32_.alloc();
        if (size <= 64)  return p64_.alloc();
        if (size <= 128) return p128_.alloc();
        return nullptr;
    }

    void free(void* ptr, size_t size) {
        if (size <= 8)   { p8_.free(ptr); return; }
        if (size <= 16)  { p16_.free(ptr); return; }
        if (size <= 32)  { p32_.free(ptr); return; }
        if (size <= 64)  { p64_.free(ptr); return; }
        if (size <= 128) { p128_.free(ptr); return; }
    }
